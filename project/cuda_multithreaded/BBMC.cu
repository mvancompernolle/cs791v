...
__global__ void maxClique(unsigned int* N, unsigned int* invN, unsigned int* solution, unsigned int* devC, unsigned int* devP);
...
std::cout << std::endl;
	...
	/*err = cudaMalloc( (void**) &devP, numInts * 100000 * sizeof(unsigned int));
	if (err != cudaSuccess) {
		std::cerr << "Error: " << cudaGetErrorString(err) << std::endl;
		exit(1);
	}
	err = cudaMalloc( (void**) &devC, numInts * 100000 * sizeof(unsigned int));
	if (err != cudaSuccess) {
		std::cerr << "Error: " << cudaGetErrorString(err) << std::endl;
		exit(1);
	}*/
